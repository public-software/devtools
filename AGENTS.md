...
This is `devtools`, one repository of [Public Software](https://publicsoftware.dev): a spec-first cleanroom Rust suite built by people and coding agents together. Read this file before the README; it is the contract every agent works under here.

**Ring:** system · **Layers:** L2 · **Wave:** 1 · Native debugger (DAP), pure-Rust fuzzer, profilers, formal-verification harness integration. Planned components: pub-dbg · pub-fuzz · pub-prof · verify harness.
...
- A new crate is `pub new <kind> <name>` with a kind from lib, app, service, plugin, spec; it lands in `crates/pub-devtools-<name>/` and gets a `[[component]]` entry in `CATALOG.toml`.

## Conventions

- Crates are named `pub-devtools-<name>`; a service is the binary `pubd-<name>`; WIT interfaces live under `public:`.
...
- The merge gates decide (`pub check`, CI, the agent review `suite / policy`, the maintainer team `public-software/maint-devtools`), not who or what wrote the change.
<!-- agents:end -->

## Repo notes

_Maintainers of `devtools` write here what the generated section cannot know: how the crates fit together, how to run the thing, which specifications matter, what not to touch. Everything above this heading is regenerated._
